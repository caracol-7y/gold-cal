...
def render_calc_results(theory, sell, rate, buy=None, buy_rate=None):
    # 計算結果（縦並び）の表示
    # 1. 最大価格
    html1 = f'<div class="ios-card" style="text-align: center; padding: 10px;"><span style="font-size: 12px; color: gray;">最大価格 (100%)</span><br><span style="font-size: 26px; font-weight: 800;">¥{theory:,.0f}</span></div>'
    st.markdown(html1, unsafe_allow_html=True)

    # 2. 割合価格
    html2 = f'<div class="ios-card" style="border: 2px solid #ff4b4b; background-color: rgba(255, 75, 75, 0.05); text-align: center; padding: 10px;"><span style="font-size: 12px; color: #ff4b4b;">割合価格 ({rate}%)</span><br><span style="font-size: 26px; font-weight: 800; color: #ff4b4b;">¥{sell:,.0f}</span></div>'
    st.markdown(html2, unsafe_allow_html=True)

    # 3. 歩金込価格（チェック時のみ）
    if buy is not None:
        html3 = f'<div class="ios-card" style="border: 2px solid #007AFF; background-color: rgba(0, 122, 255, 0.05); text-align: center; padding: 10px;"><span style="font-size: 12px; color: #007AFF;">歩金込価格 ({buy_rate})</span><br><span style="font-size: 26px; font-weight: 800; color: #007AFF;">¥{buy:,.0f}</span></div>'
