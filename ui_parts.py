...
def render_price_list(category_label, keys, prices_dict, options_map):
    """相場一覧画面：カテゴリごとの価格リスト"""
    ...
    for i, k in enumerate(keys):
        ...
        html += f'<div style="display: flex; justify-content: space-between; padding: 15px; {border}"><span>{options_map[k]}</span><span style="font-weight: 700; color: #ff4b4b;">{txt}</span></div>'
    html += '</div>'
