...
def render_calc_results(theory, sell, rate, buy=None, buy_rate=None):
    """計算結果を縦並びで表示（変数名の不一致を解消）"""
    # 1. 最大価格
    t1 = f'<div class="ios-card" style="text-align: center; padding: 10px;">'
    t1 += f'<span style="font-size: 12px; color: gray;">最大価格 (100%)</span><br>'
    t1 += f'<span style="font-size: 26px; font-weight: 800;">¥{theory:,.0f}</span></div>'
    st.markdown(t1, unsafe_allow_html=True)

    # 2. 割合価格
    t2 = f'<div class="ios-card" style="border: 2px solid #ff4b4b; background-color: rgba(255, 75, 75, 0.05); text-align: center; padding: 10px;">'
    ...
    # 3. 歩金込価格
    if buy is not None:
        t3 = f'<div class="ios-card" style="border: 2px solid #007AFF; background-color: rgba(0, 122, 255, 0.05); text-align: center; padding: 10px;">'
