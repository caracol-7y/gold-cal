...
def render_calc_results(theory, sell, rate, buy=None, buy_rate=None):
    """計算結果カード"""
    st.markdown(f"""
    <div class="ios-card" style="text-align: center; padding: 10px;">
        <span style="font-size: 12px; color: gray;">最大価格 (100%)</span><br>
        <span style="font-size: 26px; font-weight: 800;">¥{theory:,.0f}</span>
    </div>
    """, unsafe_allow_html=True)

    st.markdown(f"""
    <div class="ios-card" style="border: 2px solid #ff4b4b; background-color: rgba(255, 75, 75, 0.05); text-align: center; padding: 10px;">
        <span style="font-size: 12px; color: #ff4b4b;">割合価格 ({rate}%)</span><br>
        <span style="font-size: 26px; font-weight: 800; color: #ff4b4b;">¥{sell:,.0f}</span>
    </div>
    """, unsafe_allow_html=True)

    if buy is not None:
        st.markdown(f"""
        <div class="ios-card" style="border: 2px solid #007AFF; background-color: rgba(0, 122, 255, 0.05); text-align: center; padding: 10px;">
            <span style="font-size: 12px; color: #007AFF;">歩金込価格 ({buy_rate})</span><br>
            <span style="font-size: 26px; font-weight: 800; color: #007AFF;">¥{buy:,.0f}</span>
        </div>
        """, unsafe_allow_html=True)
...
def render_history_card(m):
    ...
    if m["buy_total"] != "-":
        ...
        <div style="display: flex; justify-content: space-between;">
            <span style="color: #007AFF; font-size: 14px;">歩金 ({m["buy_rate"]})</span>
            <span style="color: #007AFF; font-weight: 800;">{m["buy_total"]}</span>
        </div>
        """
...
def render_price_list(label, keys, prices, options_map):
    ...
    for k in keys:
        if k in prices:
            ...
            val = prices[k]
            html += f"""
            <div style="display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid rgba(128,128,128,0.1);">
                <span style="font-weight: 700;">{disp}</span>
                <span style="font-weight: 800; color: #007AFF;">¥{val:,}/g</span>
            </div>
            """
    html += '</div>'
    st.markdown(html, unsafe_allow_html=True)
